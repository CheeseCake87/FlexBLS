...
def query_read_system_user_by_private_key(private_key: str) -> SystemUser | None:
    wh_ = (SystemUser.private_key == private_key,)
    se_ = s.select(SystemUser).where(*wh_)
    re_ = db.session.execute(se_).scalar()
    return re_
...
def query_delete_system_user(
    username: str, user_id: int, _auto_commit: bool = True, _flush: bool = True
) -> int | None:
